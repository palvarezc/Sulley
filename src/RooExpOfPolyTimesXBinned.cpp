...
//ClassImp(RooExpOfPolyTimesXBinned) 
 ...
 { 
   ...
   std::string binnedFname = "binnedFunction_";
   if (name) binnedFname+= name;
   // binnedFunction = new TF1(binnedFname.c_str(),RooExpOfPolyTimesXBinnedFunction,pT_lowBound,pT_highBound,7);
   binnedFunction = new TF1("f",this,&RooExpOfPolyTimesXBinned::evaluateBin,pT_lowBound,pT_highBound,2);
...
Double_t RooExpOfPolyTimesXBinned::evaluate() const
{
   ...
   int bin_number_pT = grid->GetYaxis()->FindFixBin(pT);
   if (bin_number_pT<1 || bin_number_pT>grid->GetYaxis()->GetNbins()) return 0;
   ...
   // cout<<"Looking at bin in mass: "<<grid->GetXaxis()->FindFixBin(m)<<endl;
   // cout<<"Looking at bin in pT: "<<grid->GetYaxis()->FindFixBin(pT)<<endl;
   ...
   binnedFunction->SetParameter(0,m_min);
   binnedFunction->SetParameter(1,m_max);
   // binnedFunction->SetParameter(2,l1);
   // binnedFunction->SetParameter(3,l2);
   // binnedFunction->SetParameter(4,l3);
   // binnedFunction->SetParameter(5,l4);
   // binnedFunction->SetParameter(6,l5);
   ...
   ret = binnedFunction->Integral(pT_min,pT_max)/binWidth;
      ...
      case 1:
         {
           ...
           for(int i=1; i<m_nbins+1; i++)
           {
             ...
             binnedFunction->SetParameter(0,m_min);
             binnedFunction->SetParameter(1,m_max);
             sum += binnedFunction->Integral(pT_min,pT_max)*1./binWidth; 
           }
      ...
      case 2:
         {
           ...
           double sum(0);
           for(int i=1; i<pT_nbins+1; i++)
           {
             pT_min = grid->GetYaxis()->GetBinLowEdge(i);
             pT_max = grid->GetYaxis()->GetBinLowEdge(i+1);
             sum += binnedFunction->Integral(pT_min,pT_max)*1./binWidth; 
           }
   ...
   case 3:
     {
           ...
           for(int i=1; i<m_nbins+1; i++)
           {
             ...
             {  
               ...
               binnedFunction->SetParameter(0,m_min);
               binnedFunction->SetParameter(1,m_max);
               sum += binnedFunction->Integral(pT_min,pT_max); 
             }
           }
