...
string measurementName1D = "my_measurement";
string channelName1D = "B2Kee";


FitterUtilsHistFact1D::FitterUtilsHistFact1D(int nGenSignal_, int nGenPartReco_, int nGenComb_, int nGenJpsiLeak_, double nGenFracZeroGamma_
  ...
  :nGenSignal(nGenSignal_), nGenPartReco(nGenPartReco_), nGenComb(nGenComb_), nGenJpsiLeak(nGenJpsiLeak_), 
   ...
   nMassBins(0), binWidthMass(40.), binWidthMassBroader(80.), binWidthMassExtended(80.)
{}


void FitterUtilsHistFact1D::initiateParams(RooArgSet* parset)
{
   RooRealVar *var;
...
void FitterUtilsHistFact1D::initiateParams(int nGenSignalZeroGamma, int nGenSignalOneGamma, int nGenSignalTwoGamma, 
                                         ...
                                         RooRealVar& l1Kee, RooRealVar const& l1KeeGen, bool constFracs, bool constComb)
{
   ...
   l1Kee.setRange( l1KeeGen.getVal() - 1000*l1KeeGen.getError(), l1KeeGen.getVal() + 1000*l1KeeGen.getError() );

   }

}

TH1D* FitterUtilsHistFact1D::make_base_histogram(string componentName, string componentFile, string componentTree,
...
{  
   ...
   TH1D *hcomponent = new TH1D(componentName.c_str(),componentName.c_str(),
...
void FitterUtilsHistFact1D::fit_combinatorial(string combfile, string combtree, string combcuts, vector<string> varset, 
                                            RooRealVar &B_plus_M, RooWorkspace *workspace)
{
   ...
   RooAbsPdf *combPDF;
   // combPDF =  new RooExpBinned("combPDF", "combPDF", B_plus_M, l1Kee ,m_binning);
   combPDF =  new RooExponential("combPDF", "combPDF", B_plus_M,  l1Kee);
...
void FitterUtilsHistFact1D::prepare_PDFs(string trigStr, string weightStr, string BDTVar, double BDTcut,
      ...
      string signaltree, string partrecotree, string combtree, string JpsiLeaktree)
{
  ...
  vector<string> argsetComb = {"B_plus_DTFM_M_zero", "B_plus_M", 
                               BDTVar, trigStr, "e_plus_BremMultiplicity", "e_minus_BremMultiplicity", "K_Kst_isMuon"};
  vector<string> argsetPartReco = {"B_plus_DTFM_M_zero",  "B_plus_M", 
                                    BDTVar, trigStr, "e_plus_BremMultiplicity", "e_minus_BremMultiplicity", weightStr};
  vector<string> argsetJpsiLeak = {"B_plus_DTFM_M_zero",  "B_plus_M", 
                                   BDTVar, trigStr, "e_plus_BremMultiplicity", "e_minus_BremMultiplicity", weightStr};
  vector<string> argsetSignal = {  "B_plus_M", 
                                 BDTVar, trigStr, "e_plus_BremMultiplicity", "e_minus_BremMultiplicity", weightStr};
  ...
  TH1D* h_SignalZero = make_base_histogram("SignalZero", signalfile, signaltree, SignalZeroCuts, argsetSignal, B_plus_M, "default");
  TH1D* h_SignalOne = make_base_histogram("SignalOne", signalfile, signaltree, SignalOneCuts, argsetSignal, B_plus_M, "default");
  TH1D* h_SignalTwo = make_base_histogram("SignalTwo", signalfile, signaltree, SignalTwoCuts, argsetSignal, B_plus_M, "default");
  TH1D* h_PartReco = make_base_histogram("PartReco", partrecofile, partrecotree, PartRecoCuts, argsetPartReco, B_plus_M, "default");
  TH1D* h_JpsiLeak = make_base_histogram("JpsiLeak", JpsiLeakfile, JpsiLeaktree, JpsiLeakCuts, argsetJpsiLeak, B_plus_M, "default");
  TH1D* h_JpsiLeakbroader = make_base_histogram("JpsiLeakbroader", JpsiLeakfile, JpsiLeaktree, JpsiLeakCuts, argsetJpsiLeak, B_plus_M, "broaderBins");
  TH1D* h_Comb  = make_base_histogram("Comb", combfile, combtree, CombCuts, argsetComb, B_plus_M, "default");
  ...
  RooStats::HistFactory::Measurement meas(measurementName1D.c_str(),measurementName1D.c_str());
  ...
  RooStats::HistFactory::Channel chan(channelName1D.c_str());
  ...
  RooRealVar *B_plus_M_mod = (RooRealVar*) obs->find(("obs_x_"+channelName1D).c_str());
  ...
  RooArgList arglist(*B_plus_M_mod);
  ...
  RooDataHist dataHistJpsiLeak("dataHistJpsiLeak", "dataHistJpsiLeak", arglist, h_JpsiLeak); //h_JpsiLeakbroader
   ...
   RooArgSet argset2(*B_plus_M_mod);
...
void FitterUtilsHistFact1D::generate(bool wantPlots, string plotsfile)
{
  ...
  TFile* fw = TFile::Open(workspacename.c_str(), "UPDATE");   
   ...
   RooArgSet *obs = (RooArgSet*) mc->GetObservables();
   RooRealVar *B_plus_M = (RooRealVar*) obs->find(("obs_x_"+channelName1D).c_str());
   ...
   // combPDF =  new RooExpBinned("combPDF", "combPDF", *B_plus_M, *l1KeeGen, m_binning);

   combPDF =  new RooExponential("combPDF", "combPDF", *B_plus_M, *l1KeeGen);
...
void FitterUtilsHistFact1D::fit(bool wantplot, bool constPartReco,
      ...
      ofstream& out, TTree* t, bool update, string plotsfile)
{
  ...
  //***************** Build model
   ...
   RooBinning m_binning(nMassBins,B_plus_M->getMin(),B_plus_M->getMax(),"m_binning");
   RooExpBinned *combPDF_unnorm =  new RooExpBinned("combPDF_unnorm", "combPDF_unnorm", 
                                                 *B_plus_M, *l1Kee, m_binning);

   RooAbsReal *combIntegral = combPDF_unnorm->createIntegral(RooArgSet(*B_plus_M));
   ...
   RooFormulaVar *combPDF = new RooFormulaVar("combPDF","combPDF_unnorm/combIntegral",RooArgList(*combPDF_unnorm,*combIntegral));  
   ...
   // Combined model
   RooRealSumPdf *model_pdf = new RooRealSumPdf((channelName1D+"_comb").c_str(),(channelName1D+"_comb").c_str(),
   ...
   RooProdPdf *model_pdf_constrain = new RooProdPdf("RK_kinematic_comb_constrain","RK_kinematic_comb_constrain",
                                                    *model_and_constraints);
   ...
   RooDataHist *dataGenTot = new RooDataHist("dataGenTot","dataGenTot",RooArgSet(*B_plus_M),
  ...
  // nJpsiLeak->setConstant(kTRUE);
   ...
   initiateParams(nGenSignalZeroGamma, nGenSignalOneGamma, nGenSignalTwoGamma, 
                  ...
                  *l1Kee, *l1KeeGen, constFracs, constComb);
   ...
   double integralcomb = combPDF->createIntegral(RooArgSet(*B_plus_M))->getVal();

   cout<<"Integral of combPDF: "<<integralcomb<<endl;
   
   
   if(wantplot) plot_fit_result(plotsfile, totPdf, dataGenTot, 
                                // model_pdf->createIntegral(RooArgSet(*B_plus_M))->getVal());
                                // nSignal->getVal() + nPartReco->getVal() + nComb->getVal()*integralcomb + nJpsiLeak->getVal());
                                nSignal->getVal(), nPartReco->getVal(), nComb->getVal(), nJpsiLeak->getVal(),
                                fracZero->getVal(),(1-fracZero->getVal())*(fracOne->getVal()));
...
void FitterUtilsHistFact1D::plot_fit_result(string plotsfile, RooSimultaneous &totPdf, RooDataHist *dataGenTot,
                                            double nsignal, double npartreco, double ncomb, double njpsileak,
                                            double fzero, double fone)
{

  double ntot = nsignal + npartreco + ncomb + njpsileak;
  double ftwo = 1-fzero-fone;
   ...
   while((var = (RooRealVar*) iter->Next()))
   {
     ...
     if (strcmp(var->ClassName(),"RooCategory")==0) continue;
      ...
      totPdf.plotOn(frame,Name("model"), Slice(*idx),ProjWData(*idx,*dataGenTot),
                    DrawOption("F"),FillColor(kGray),
                    LineWidth(0));
      ...
      totPdf.plotOn(frame,Name("comb_line"), Slice(*idx),ProjWData(*idx,*dataGenTot), 
                    ...
                    Normalization(ncomb,RooAbsReal::NumEvent));
                    
      totPdf.plotOn(frame,Name("leak"), Slice(*idx),ProjWData(*idx,*dataGenTot),
                    DrawOption("F"),FillColor(kCyan),Components("*Zero*,*One*,*Two*,*PartReco*,*Leak*"),
                    LineWidth(0),Normalization(nsignal+npartreco+njpsileak,RooAbsReal::NumEvent));
      ...
      totPdf.plotOn(frame,Name("partreco"), Slice(*idx),ProjWData(*idx,*dataGenTot),
                    ...
                    LineWidth(0),Normalization(nsignal+npartreco,RooAbsReal::NumEvent));
      
      totPdf.plotOn(frame,Name("signal"), Slice(*idx),ProjWData(*idx,*dataGenTot),
                    DrawOption("F"),FillColor(kBlue), Components("*Zero*,*One*,*Two*"),
                    LineWidth(0), Normalization(nsignal,RooAbsReal::NumEvent));

      totPdf.plotOn(frame,Name("sigzero"), Slice(*idx),ProjWData(*idx,*dataGenTot),Components("*Zero*"),
                    LineWidth(1),LineColor(kYellow),Normalization(nsignal*fzero,RooAbsReal::NumEvent));

      totPdf.plotOn(frame,Name("sigone"), Slice(*idx),ProjWData(*idx,*dataGenTot),
                    LineColor(kViolet),Components("*One*"),LineWidth(1),Normalization(nsignal*fone,RooAbsReal::NumEvent));

      totPdf.plotOn(frame,Name("sigtwo"), Slice(*idx),ProjWData(*idx,*dataGenTot),
                    LineColor(kGreen),Components("*Two*"),LineWidth(1),Normalization(nsignal*ftwo,RooAbsReal::NumEvent));
