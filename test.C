...
string channelName = "B2Kee";
 int nGenJpsiLeak = 10;
  ...
  RooMinuit* minuit_hf = new RooMinuit(*nll) ;
  ...
  RooPlot *m_frame = B_plus_M->frame(Title("M(B)"));
   ...
   totPdf.plotOn(m_frame,Name("model"), Slice(*idx),ProjWData(*idx,*dataGenTot),DrawOption("F"),FillColor(kRed),
                 LineWidth(0));
  ...
  RooCategory *cat = (RooCategory*) (&totPdf.indexCat());
  cout<<"category: "<<cat->GetName();
