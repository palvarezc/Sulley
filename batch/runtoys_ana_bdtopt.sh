...
for TRIGGER in $(echo 0 1 2);do 
    for MODEINDX in $(echo 0);do
	echo "Submitting trigcat "$TRIGGER

	YIELDSFILE="/home/hep/palvare1/public_html/BDT_optimisation/yields_trigCat"$TRIGGER".dat"
	BDTINDX=0	
	MODE=${MODE_CODES[MODEINDX]}	
	BDT_VAR_NAME=${BDT_VAR_NAMES_1D_RUN1[$TRIGGER]}	    
 
	sed 1d $YIELDSFILE | sed 1d | while read -r line
	do
	    OUTPUTDIR=$OUTPUT/Run$RUN/Trig$TRIGGER/${MODE_NAMES[MODEINDX]}/BDT$BDTINDX/
	    ...
	    BDT_CUT=${array[0]} 
	    Y_SIG=${array[1]} 
	    Y_PART_RECO=$(echo "scale=0;"${PART_RECO_FRACS[TRIGGER]}"*"$Y_SIG | bc)
	    Y_JPSILEAK=${array[2]} 
	    Y_COMB=${array[3]} 
	

	
	    Y_SIG=`printf '%.0f\n' $Y_SIG` #makes the yield integer
	    Y_PART_RECO=`printf '%.0f\n' $Y_PART_RECO` #makes the yield integer
	    Y_JPSILEAK=`printf '%.0f\n' $Y_JPSILEAK` #makes the yield integer
	    Y_COMB=`printf '%.0f\n' $Y_COMB` #makes the yield integer

	    echo "Configuration - "$BDT_CUT" "$Y_SIG" "$Y_PART_RECO" "$Y_JPSILEAK" "$Y_COMB	
